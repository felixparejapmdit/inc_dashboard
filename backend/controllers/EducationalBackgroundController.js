...
module.exports = {
  ...
  getEducationByPersonnelId: async (req, res) => {
    try {
      const education = await EducationalBackground.findAll({
      ...
      if (!education || education.length === 0) {
        return res
          .status(404)
          .json({ message: "No educational records found." });
      }

      res.status(200).json(education);
    } catch (error) {
      console.error("Error fetching educational background:", error.message);
      res
        .status(500)
        .json({ error: "Failed to fetch educational background." });
    }
  },
  ...
  updateEducationalBackground: async (req, res) => {
    ...
    try {
      ...
      await existingRecord.update({
        ...
        certificate_files: certificate_files
          ? JSON.stringify(certificate_files)
          : existingRecord.certificate_files, // Update JSON
        updated_at: new Date(),
      });
  ...
  updateEducationalBackgroundWithCertificates: async (req, res) => {
    ...
    try {
      ...
      if (!existingRecord) {
        return res
          .status(404)
          .json({ message: "Educational background not found" });
      }
      ...
      await existingRecord.update({
        certificate_files: certificates
          ...
          : existingRecord.certificate_files,
      });

      res.status(200).json({
        message:
          "Educational background updated with certificates successfully",
        data: existingRecord,
      });
    } catch (error) {
      console.error(
        "Error updating educational background with certificates:",
        error.message
      );
      res.status(500).json({
  ...
  removeCertificate: async (req, res) => {
    ...
    if (!filePath || !educationId) {
      ...
      return res
        .status(400)
        .json({ message: "File path and education ID are required." });
    }
    ...
    try {
      ...
      const educationalBackground = await EducationalBackground.findByPk(
        educationId
      );
      ...
      try {
        certificateFiles = JSON.parse(
          educationalBackground.certificate_files || "[]"
        );
      } catch (parseError) {
